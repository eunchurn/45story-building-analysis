...
for kk=1:12
    subplot(3,4,kk)
    plot([0;phi(1:45,135-kk+1)],ST,'-k',[0;phi(46:90,135-kk+1)],ST,'-r',[0;phi(91:135,135-kk+1)],ST,'-b')
    xlabel('Mode Shape'),ylabel('Storey')
    legend(['Mode ',num2str(kk)])
    ylim([0 45])
end

figure
subplot(2,2,1)
plot([0;phi(1:45,134)],ST,'*-k',-[0;phi(46:90,135)],ST,'*-r',-[0;phi(91:135,133)],ST,'*-b','LineWidth',1.5)
xlabel('Mode Shape'),ylabel('Storey')
legend('X-Translation','Y-Translation','Torsional')
ylim([0 45])
subplot(2,2,2)
plot([0;phi(1:45,132)],ST,'*-k',[0;phi(46:90,131)],ST,'*-r',-[0;phi(91:135,130)],ST,'*-b','LineWidth',1.5)
xlabel('Mode Shape'),ylabel('Storey')
legend('X-Translation','Y-Translation','Torsional')
ylim([0 45])
subplot(2,2,3)
plot([0;phi(1:45,129)],ST,'*-k',-[0;phi(46:90,128)],ST,'*-r',[0;phi(91:135,127)],ST,'*-b','LineWidth',1.5)
xlabel('Mode Shape'),ylabel('Storey')
legend('X-Translation','Y-Translation','Torsional')
ylim([0 45])
subplot(2,2,4)
plot(-[0;phi(1:45,126)],ST,'*-k',[0;phi(46:90,124)],ST,'*-r',[0;phi(91:135,124)],ST,'*-b','LineWidth',1.5)
xlabel('Mode Shape'),ylabel('Storey')
legend('X-Translation','Y-Translation','Torsional')
ylim([0 45])
% figure
% for kk=1:12
%     subplot(3,4,kk)
%     plot([0;phi(1:45,135-kk+1)],ST,'-k')
%     xlabel('X-dir Mode Shape'),ylabel('Storey')
%     legend(['Mode ',num2str(kk)])
%     ylim([0 45])
% end
% figure
% for kk=1:12
%     subplot(3,4,kk)
%     plot([0;phi(46:90,135-kk+1)],ST,'-r')
%     xlabel('Y-dir Mode Shape'),ylabel('Storey')
%     legend(['Mode ',num2str(kk)])
%     ylim([0 45])
% end
% figure
% for kk=1:16
%     subplot(4,4,kk)
%     plot([0;phi(91:135,135-kk+1)],ST,'-b')
%     xlabel('Torsional Mode Shape'),ylabel('Storey')
%     legend(['Mode ',num2str(kk)])
%     ylim([0 45])
% end
...
for kk=1:length(t)
    ...
    x=F*x+Bds*(nin(kk));
    P=F*P*F'+Bds*Q*Bds';
    waitbar(kk/length(t),h,'Kalman Filtering...')
    xe(:,kk)=x;
end
close(h)
%% 층간변위 기준값
...
figure,plot(t,d45,'-k',t,xe(45,:),':r')
xlim([t(1) t(end)])
xlabel('Time (second)'),ylabel('Displacement of 45F (m)')
legend('Numerical','Estimation')
nfft=2^nextpow2(length(t));
[Pa,Fa]=pwelch(d45,[],[],nfft,1000);
[Pe,Fe]=pwelch(xe(45,:),[],[],nfft,1000);
figure,loglog(Fa,Pa,'-k',Fe,Pe,':r'),grid on
xlabel('Frequency (Hz)'),ylabel('PSD of 45F Displacement')
legend('Numerical','Estimation')

figure,barh(max(abs(diff(xe(1:45,:))')),'k'),xlabel('Maximum Dynamic Story Drift (m)'),ylabel('Story (Floor)')
% figure,bar(max(abs(diff(Xo(1:45,:)'))),'k')